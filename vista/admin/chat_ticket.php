...
switch($rol_usuario) {
    case 'administrador':
        $url_retorno = '/proyectophp/vista/admin/?url=gestion_tickets';
        break;
    case 'tecnico':
        $url_retorno = '/proyectophp/vista/tecnico/mis_tickets.php';
        break;
    case 'docente':
        $url_retorno = '/proyectophp/vista/usuario/?url=mis_tickets';
        break;
    default:
        $url_retorno = '/proyectophp/index.php';
}
...
if($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['cambiar_estado'])){
    ...
    $resultado = $controladorTicket->actualizarEstado($id_ticket, $nuevo_estado, $id_usuario);
    
    if($resultado['success']){
        header("Location: chat_ticket.php?id=$id_ticket&estado_actualizado=1");
        exit;
    } else {
...
<body>
    <div class="container">
        <a href="<?php echo $url_retorno; ?>" class="volver">
            ← Volver a <?php echo ($rol_usuario === 'administrador') ? 'gestión de tickets' : 'mis tickets'; ?>
        </a>
        ...
        <?php if(isset($_GET['estado_actualizado'])): ?>
            <div class="alerta alerta-exito">Estado del ticket actualizado correctamente</div>
        <?php endif; ?>
        
        <!-- Control de Estado (Solo para Administradores) -->
        <?php if($rol_usuario === 'administrador'): ?>
        <form method="POST" class="estado-control">
            ...
            <select name="nuevo_estado" id="nuevo_estado" required>
                ...
                <option value="en_progreso" <?php echo $ticket['estado'] == 'en_progreso' ? 'selected' : ''; ?>>En Progreso</option>
            ...
            <button type="submit" name="cambiar_estado" class="btn-cambiar-estado">
